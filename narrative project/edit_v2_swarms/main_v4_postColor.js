...
let filteredData;
let unpackedData;
let days_since_first_announcement;
let days_since_first_case;
let regions_lookup = {};
...
// Initialize colors for different schemes -- 
...
function init () {
    ...
    legend_svg = d3.select("#legend")
        .append("svg").attr("width",width/2).attr("height", 100);
    ...
    let xAxis_titleTop = swarm.append("text")
        .attr("class","x-axis-title-top")
        .attr("transform",`translate(${xScale(0)},${margins.top-30})`)
        .style("font","14px sans-serif")
        .style("font-weight","bold")
        .style("font-family","Avenir")
        .text("Days since 1st case of COVID-19 in country");

    let xAxis_titleBottom = swarm.append("text")
        .attr("class","x-axis-title-bottom")
        .attr("transform",`translate(${xScale(0)},${height-5})`)
        .style("font","14px sans-serif")
        .style("font-weight","bold")
        .style("font-family","Avenir")
        .text("Days since 1st case of COVID-19 in country");
...
// establish the region lookup, if we want to use this later for shading...
// or, for filtering
...
var tooltip = d3.select("#plots_container").append("div")
.attr("class", "tooltip")
.style("opacity", 0);

// tooltip mouseover event handler
var tipMouseover = function(d) {
    var color = colorScale(d[index_vars[state.selectedIndex]]);
