...
Promise.all([
    ...
    d3.csv("./data/regyear_termMents_stackedready.csv",d3.autoType),
...
/// init and scales for the stacked chart
    ...
    stacked_plot_area = stacked_plot.append("g")
    .attr("class","stacked_plot_area");


    // Add X axis
    yScale_stacked = d3.scaleBand()
        .domain(["Latin_America_and_the_Caribbean","Global",
        "Asia_Pacific","Middle_East_and_North_Africa","US_Canada","Europe","Africa","Eurasia"])
        .range([height/2 - margin.bottom, margin.top-10]);

    xScale_stacked = d3.scaleLinear()
        .domain([0,1])
        .range([ margin.left, width/2]);

    const yAxis_stacked = d3.axisLeft(yScale_stacked)
        .ticks(8)
        .tickSizeInner(5)
        .tickSizeOuter(5);

    stacked_plot_area.append("g")
        .attr("class", "stacked_plot axis--x")
        .attr("transform", `translate(0, ${margin.bottom-5})`)
        .call(d3.axisBottom(xScale_stacked));
    
    stacked_plot_area.append("g")
        .attr("class", "stacked_plot axis--y")
        .attr("transform", `translate(${margin.left}, 0)`)
        .call(d3.axisLeft(yScale_stacked));
...
function drawStacked () {
    ...
    var stack = d3.stack()
        .keys([2016,2017,2018,2019])
        .order(d3.stackOrderNone)
        .offset(d3.stackOffsetNone);

    var series = stack(stackedFiltered);

    var serie = stacked_plot_area.selectAll(".serie")
        .data(series)
        .enter().append("g")
        .attr("class", "serie")
    ...
    serie.selectAll("rect")
        ...
        .join(enter =>
            ...
            .append("rect")
            .attr("class","rect")
            .attr("id",d=>d.data.obs_region+state.hover.term)
            .attr("y", function(d) {return yScale_stacked(d.data.obs_region); })
            .attr("x", function(d) {console.log(xScale_stacked(d[1])); return xScale_stacked(d[0]); })
            .attr("width", function(d) { return xScale_stacked(d[1]) - xScale_stacked(d[0]); })
            .attr("height", yScale_stacked.bandwidth())
            .call(enter =>
                enter
                .transition()
                .duration(500)),
