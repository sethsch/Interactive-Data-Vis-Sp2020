...
Promise.all([
    ...
    d3.csv("./data/filtered_KWIC.csv", d3.autoType),
...
function tooltipOver() {
    ...
    state.selectedYear1 + ": " + state.hover.year1_val + "</span><br/>" +
    "<span style='color:" + color[state.selectedYear2] + "'>" + 
    state.selectedYear2 + ": " + state.hover.year2_val + "</strong></span>"
        )
    // moves with mouse    	
    //.style("left", (state.hover.toolXpos) + "px")		
    //.style("top", (d3.event.pageY - 28) + "px");	
    //or, stays in convenient position
    .style("left", xScale(0.60) + "px")
...
d3.selectAll(".col_header")
    ...
    .data(kwicFiltered.slice())
    .join("tr");

    /*var rows = table
    .append("tbody")
    .selectAll("tr")
    .data([["testterm"," ",0.2655," ","testterm2"," ",0.344]])
    .join("tr");
