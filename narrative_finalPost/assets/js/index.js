...
let story = {'#health_monitor': {"region":'South Asia'},
                '#public_aw': {"region":'Europe & Central Asia'},
                '#other_pol': {"region":'Latin America & Caribbean'},
                '#health_rsrc': {"region":'South Asia'},
                '#task_fc': {"region":'Europe & Central Asia'},
                '#ext_border': {"region":" All"},
                '#int_border': {"region":" All"},
                '#mass_gath': {"region":'Europe & Central Asia'},
                '#gov_serv': {"region":'Latin America & Caribbean'},
                '#emergency': {"region":'South Asia'},
                '#schools': {"region":'Europe & Central Asia'},
                '#business': {"region":'South Asia'},
                '#health_test': {"region":'South Asia'},
                '#soc_dist': {"region":'Europe & Central Asia'},
                '#lockdown': {"region":'Europe & Central Asia'},
                '#curfew': {"region":'South Asia'},
                '#hygeine': {"region":'Europe & Central Asia'},
                '#disinfo': {"region":'Latin America & Caribbean'}}
...
Promise.all([
    ...
    d3.json("./assets/data/ALL_countries_covid_May16.json", d3.autotype),
...
function init() {
  ...
  for (var x = 0; x < divs.length; x++) {
    let newPlot = new SwarmChart(state,setGlobalState,poltypes[x],divs[x])
    swarms.push(newPlot)
  }

  
  /// setting up a slider, epxeirmenting w log scale
  ...
  /*d3.select('p#value-range').text(
    ...
    selectCountries = d3.selectAll("#countries_dropdown").on("change", function() {
  ...
  selectCountries.property("value", " All");*/
    ...
    let country_list = Object.keys(state.countrydata);
    region_list = ["South Asia", "Europe & Central Asia",
    ...
    selectCountries.append("optgroup")
    .attr("class","All countries").attr("label"," All countries").attr("order",1)
    ...
    // 2. dropdown for regions for each div
    selectRegion = d3.selectAll("#region_dropdown")
      //.attr("id","region_dropdown_"+divs[x]) // maybe dropdowns need to be separate for each section to get story functionality?
      .on("change", function() {
  ...
  let reg_vals = [" All","South Asia", "Europe & Central Asia",
    "Middle East & North Africa","Sub-Saharan Africa","Latin America & Caribbean",
    "East Asia & Pacific","North America"];

    reg_vals = reg_vals.sort(d3.ascending);
...
function draw() {
  ...
  for (var i = 0; i < divs.length; i++) {
    //state.selectedRegion = story[divs[i]].region;
    //state.selectedCountry = defaultCountries;
    //selectRegion.select(".region_dropdown_"+divs[i]).property("value", state.selectedRegion);
    //selectCountries.property("value",state.selectedCountry);
